...
- **회원 가입 및 로그인**
    ...
    가입 시 기본 인적 사항을 입력 후 주요 관심사 선택( skip 가능 ) 해서 가입. ( 추후 매칭에 활용 )
    
    이메일, 휴대폰 번호 또는 소셜 로그인 옵션을 통해 로그인. ( 소셜 로그인은 API 고려 )
    
- **프로필 관리 ( 프로필 미 작성 시 다른 동작 불가? )**
...
- **~~푸시 알림~~**
    ...
    ~~회원은 모임 일정, 메시지, 댓글, 공지사항 등의 실시간 푸시 알림을 받을수 있다.~~
    
- **~~알림 설정~~**
    
    ~~회원은 개인별로 알림 설정 및 관리 할수있다.~~
---
